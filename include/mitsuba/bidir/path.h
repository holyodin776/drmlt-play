...
public:
    ...
    void reserve(const size_t capacity = 16)
    {
        m_vertices.reserve(capacity);
        m_edges.reserve(capacity - 1);
    }
    void resize(const size_t count, MemoryPool &pool)
    {
        if (count <= m_vertices.size())
            return;

        if (m_vertices.size() == 0)
            append(pool.allocVertex());
        for (size_t i = m_vertices.size(); i < count; ++i)
            append(pool.allocEdge(), pool.allocVertex());
        SAssert(m_vertices.size() == count && length() == m_vertices.size() - 1);
    }
    ...
    int randomWalk(const Scene *scene, Sampler *sampler,
                   ...
                   MemoryPool &pool, bool adjointComp = true, bool gvpm = false);
    ...
    int randomWalkFromPixel(const Scene *scene, Sampler *sampler,
                            ...
                            MemoryPool &pool,
                            const bool unidirectional = false, const std::pair<Point2, Point2>* fixedsample = nullptr);
    ...
    static std::pair<int, int> alternatingRandomWalkFromPixel(const Scene *scene,
                                                              ...
                                                              Path &sensorPath, int nSensorSteps,
    ...
    static Float miWeightGradNoSweep_GBDPT(const Scene *scene,
                                           ...
                                           Float jDet, Float exponent, double geomTermX, double geomTermY, int maxT,
                                           float th);
    ...
    inline PathVertexPtr &vertex(size_t index) {
        ...
        if (index >= m_vertices.size())
            SLog(EError, "Path vertex index "
                    SIZE_T_FMT
                    " is out of bounds, array size: "
                    SIZE_T_FMT,
    ...
    inline const Point2 getSamplePosition(const bool reverse = false) const {
        const PathVertexPtr sensorsample = vertexOrNull(reverse ? 1 : (length()-1));
        return sensorsample ? sensorsample->getSamplePosition() : Point2(0.0);
    }
